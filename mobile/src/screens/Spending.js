...
export default function Spending() {
  ...
  if (!summary) return <View style={{ flex: 1, backgroundColor: C.paper }} />;
  ...
  return (
    ...
    >
      ...
      <Card>
        ...
        <Text style={st.big}>{naira(summary.monthTotal)}</Text>
      ...
      {suggestions.map((s, i) => (
        <Card key={s.id} style={i === 0 ? { borderColor: C.royal, borderWidth: 1.5 } : null}>
        ...
        <Card key={c.category}>
          <View style={{ flexDirection: "row", alignItems: "center" }}>
            ...
            <View style={{ alignItems: "flex-end" }}>
              ...
              {c.overByPct !== null && (
                <Text style={{ fontSize: 11, marginTop: 2, color: c.overByPct > 10 ? C.live : C.royal }}>
        ...
        <>
          ...
          {unknowns.map((tx) => (
            <Card key={tx._id}>
              ...
              {tagged[tx._id] ? (
                <Text style={{ color: C.royal, fontSize: 12.5, marginTop: 8, fontWeight: "600" }}>
                  ✓ Got it — {tx.counterparty} = {tagged[tx._id]} from now on.
                </Text>
              ) : (
                <View style={{ flexDirection: "row", marginTop: 10, flexWrap: "wrap" }}>
                  {TAG_OPTIONS.map((opt) => (
                    <TouchableOpacity key={opt} style={st.tap} onPress={() => onTag(tx, opt)}>
...
const st = StyleSheet.create({
  ...
  tap: {
    ...
    paddingVertical: 8, paddingHorizontal: 13, marginRight: 8, marginBottom: 6,
    backgroundColor: "#fff",
  },
});
